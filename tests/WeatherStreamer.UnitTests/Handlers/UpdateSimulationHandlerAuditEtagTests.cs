...
public class UpdateSimulationHandlerAuditEtagTests
{
    [Fact]
    public async Task HandleAsync_Records_PrevAndNewEtag()
    {
        // Arrange
        var originalRow = new byte[] { 1, 2, 3, 4 };
        var newRow = new byte[] { 9, 9, 9, 9 };
        ...
        var simulation = new Simulation
        {
            ...
            RowVersion = originalRow
        };

        var repo = new FakeSimulationRepository(simulation, newRow);
        ...
        var cmd = new UpdateSimulationCommand
        {
            Id = simulation.Id,
            Name = "After",
            IfMatch = Convert.ToBase64String(originalRow)
        };
        ...
        audit.PrevETag.Should().Be(Convert.ToBase64String(originalRow));
        audit.NewETag.Should().Be(Convert.ToBase64String(newRow));
    }
    ...
    class FakeSimulationRepository : ISimulationRepository
    {
        ...
        private readonly byte[] _newRowVersion;

        public FakeSimulationRepository(Simulation entity, byte[] newRowVersion)
        {
            _entity = entity;
            _newRowVersion = newRowVersion;
        }
        ...
        public Task<Simulation> UpdateAsync(Simulation entity, byte[] ifMatch, CancellationToken cancellationToken = default)
        {
            ...
            // apply changes and set synthetic new rowversion
            _entity.Name = entity.Name;
            _entity.RowVersion = _newRowVersion;
        ...
        {
            simulation.Id = _entity.Id;
            _entity = simulation;
            return Task.FromResult(simulation);
        }

        public Task<bool> IsFileInUseAsync(string filePath, CancellationToken cancellationToken = default) => Task.FromResult(false);
